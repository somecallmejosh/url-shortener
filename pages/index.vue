<template>
  <section>
    <div class="call-to-action">
      ...
      <form action class="container-max" @submit.prevent="getShortly()">
    ...
    <div class="info" :class="{'no-results' : !shortlyResults}">
      ...
      <div class="teasers container-max">
        <card :cardData="teasers"></card>
      </div>
    </div>
  </section>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      teasers: [
        ...
        {
          header: "Full Customizeable",
          text:
            "Improve brand awareness and content disoverability through customizable links, supercharging audience engagement.",
  ...
  created() {
    if (process.browser) {
      const localStorageForm = localStorage.shortly;
      if (localStorageForm) {
        const localStorageFormData = JSON.parse(localStorageForm);
        this.shortlyLinks = localStorageFormData;
        this.shortlyResults = true;
      }
    }
  },
  methods: {
...
@media (min-width: 1024px) {
  ...
  form {
    background: var(--very-dark-blue) url(/images/bg-shorten-desktop.svg) top
      right no-repeat;
    display: grid;
